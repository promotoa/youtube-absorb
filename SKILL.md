...
1. **제안은 영상 내용에서 도출한다** (붙박이 메뉴 금지). 이 영상이 남긴 것이 무엇인가:
   - **절차·도구를 보여줬다** → 그걸 사용자 환경에 들이는 제안 — "영상의 X 절차, 지금 이
     환경에 세팅해드릴까요?" 스킬을 쓰는 사용자면 스킬 흡수("영상이 소개한 그 스킬을 검토해
     설치할까요?")·스킬 창조("이 절차를 반복해 쓰시게 스킬로 만들어드릴까요?")가 그 옷이다.
   - **주장·수치를 남겼다** → 검증 제안 — "핵심 주장 N개, 실제로 맞는지 확인해볼까요?"
   - **인사이트·관점을 남겼다** → 재사용 제안 — "핵심만 담은 정리 파일(HTML/마크다운)로
     만들어둘까요?" · 사용자의 보이는 작업과 닿으면 적용 대조 — "지금 하시는 Y에 대조해볼까요?"
