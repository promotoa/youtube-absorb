...
1. **겉면 확인(전사 전·URL 단계)** — `yt-dlp --dump-json`(다운로드 없는 메타 조회) 또는 웹 접근으로
   제목·설명란·채널의 다른 영상 목록만 보고 신뢰도 신호 3문을 돈다:
...
M = **선별해 정독한 프레임 수**·대체 경로였으면 형태를 병기["150장(저해상 축소판)"]).
중심은 ②절이고, 길이는 영상 규모에 비례한다(20분 영상에 보고가 세 화면을 넘으면
요약이 아니라 이사다).

**절 번호는 표기다**: ⑤절을 생략하면 다음 스텝을 ⑤로 붙여 내보낸다 — 사용자에게 빠진 번호를
보이지 않는다(①②③④⑥은 오탈자로 읽힌다). **강등이 있었으면 헤더가 아니라 보고 첫 줄**에서
한 문장으로 밝힌다("화면 판독은 이번 환경에서 못 했습니다 — 전사 기반 보고입니다").
